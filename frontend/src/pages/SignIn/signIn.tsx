...
const SignIn: React.FC = () => {
  ...
  const handleSubmit = useCallback( 
    async (values: FormikValues) => {
      try {
        ...
        const formattedDate = formatSelectedDate(values.birthdate);
  ...
  }, [history, formatSelectedDate, image]);
  ...
  const handlePhotoChange = useCallback(
    (e: ChangeEvent<HTMLInputElement>) => {
      if (e.target.files) {
        ...
        return setImage(selectedImage);

        // api.patch('/users/:id/photo', data);
      }
    },
    [],
  );
